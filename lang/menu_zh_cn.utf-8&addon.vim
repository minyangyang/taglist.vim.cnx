...
"commnet
if g:loaded_nerd_comments
menutrans &comment                      注释(&C)
menutrans Comment                       添加注释
menutrans Toggle			切换注释
menutrans Minimal			最简注释
menutrans Nested			嵌套注释
menutrans To\ EOL			注释到尾
menutrans Invert			反转注释
menutrans Sexy				性感注释
menutrans Yank\ then\ comment		复制注释
menutrans Append			附加注释
menutrans Left\ aligned			左边对齐
menutrans Left\ and\ right\ aligned	两边对齐
menutrans Uncomment			取消注释
menutrans Switch\ Delimiters		换分隔符
menutrans Insert\ Comment\ Here		插入注释
endif

"fencview
menutrans Encodin&g			文件编码(&g)
menutrans &Auto\ Detect                 自动检测(&A)
menutrans &Toggle\ Encoding\ list       切换编码(&T)
menutrans &Input\.\.\.			输入编码(&I)



"mru
menutrans &Recent\ Files		曾用文件(&R)
menutrans  Refresh\ list		刷新列表
